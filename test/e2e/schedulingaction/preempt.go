...
var _ = ginkgo.Describe("Job E2E Test", func() {
	...
	ginkgo.It("schedule high priority job without preemption when resource is enough", func() {
	...
	ginkgo.It("schedule high priority job with preemption when idle resource is NOT enough but preemptee resource is enough", func() {
	...
	ginkgo.It("preemption doesn't work when podgroup is pending due to insufficient resource", func() {
		ctx = e2eutil.InitTestContext(e2eutil.Options{
			PriorityClasses: map[string]int32{
				highPriority: highPriorityValue,
				lowPriority:  lowPriorityValue,
	...
	ginkgo.It("preemption only works in the same queue", func() {
		ctx = e2eutil.InitTestContext(e2eutil.Options{
	...
	ginkgo.It("preemption doesn't work when total resource of idle resource and preemptee is NOT enough", func() {
		ctx = e2eutil.InitTestContext(e2eutil.Options{
			Queues: []string{"q1-preemption", "q2-reference"},
			PriorityClasses: map[string]int32{
				highPriority: highPriorityValue,
	...
	ginkgo.It("multi-preemptor-jobs who are in different priority", func() {
		ginkgo.Skip("https://github.com/volcano-sh/volcano/issues/911")
		ctx = e2eutil.InitTestContext(e2eutil.Options{
	...
	ginkgo.It("Jobs unschedulable due to scheduling gates will not preempt other jobs despite sufficient preemptor", func() {
		// Remove enqueue action first because it conflicts with preempt.
		cmc := e2eutil.NewConfigMapCase("volcano-system", "integration-scheduler-configmap")
		cmc.ChangeBy(func(data map[string]string) (changed bool, changedBefore map[string]string) {
			vcScheConfStr, ok := data["volcano-scheduler-ci.conf"]
