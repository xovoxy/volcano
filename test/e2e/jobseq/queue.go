...
var _ = ginkgo.Describe("Queue E2E Test", func() {
	ginkgo.It("Queue Command Close And Open With State Check", func() {
		...
		ginkgo.By("Close queue command check")
		err := util.CreateQueueCommand(ctx.Vcclient, defaultQueue, q1, busv1alpha1.CloseQueueAction)
		if err != nil {
			gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Error send close queue command")
		}
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Wait for closed queue %s failed", q1)

		ginkgo.By("Open queue command check")
		err = util.CreateQueueCommand(ctx.Vcclient, defaultQueue, q1, busv1alpha1.OpenQueueAction)
		if err != nil {
			gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Error send open queue command")
		}
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Wait for reopen queue %s failed", q1)
